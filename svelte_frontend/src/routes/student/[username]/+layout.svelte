<script lang="ts">
    ...
    const goToSearchTests = () => {
        if (userSub) {
            window.location.href = `/student/${userSub}/tests`;
        }
    };

    const goToTests = () => {
        if (userSub) {
            window.location.href = `/student/${userSub}/results`;
        }
    };

    const goToProfile = () => {
        if (userSub) {
            window.location.href = `/student/${userSub}`;
        }
    };
</script>
...
<div class="layout">
    <header>
        ...
        <div class="header-buttons-left">
            ...
            <button class="button button-left" on:click={goToSearchTests}>Testų paieška</button>
            <button class="button button-left" on:click={goToTests}>Mano rezultatai</button>
        </div>
